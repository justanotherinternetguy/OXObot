...
function getCharDataFromPixels(x, y) {
  ...
  var // add global cell position

    charX = Math.floor(mpX / cellW);
  charY = Math.floor(mpY / cellH);
  if (
    mpY / cellH < charY + 0.25 ||
    (mpY / cellH > charY + 0.75 && mpX / cellW < charX + 0.25) ||
    mpX / cellW < charX + 0.75
  ) {
    edge = true;
  }
  ...
  if (locX <= 0.5 && locY <= 0.25) {
    cellInnerCoords = [black, white, white, white, white, white, white, white];
  }
  if (locX <= 0.5 && locY > 0.25 && locY <= 0.5) {
    cellInnerCoords = [white, white, black, white, white, white, white, white];
  }
  if (locX <= 0.5 && locY > 0.5 && locY <= 0.75) {
    cellInnerCoords = [white, white, white, white, black, white, white, white];
  }
  if (locX <= 0.5 && locY > 0.6) {
    cellInnerCoords = [white, white, white, white, white, white, black, white];
  }
  if (locX > 0.5 && locY <= 0.25) {
    cellInnerCoords = [white, black, white, white, white, white, white, white];
  }
  if (locX > 0.5 && locY > 0.25 && locY <= 0.5) {
    cellInnerCoords = [white, white, white, black, white, white, white, white];
  }
  if (locX > 0.5 && locY > 0.5 && locY <= 0.75) {
    cellInnerCoords = [white, white, white, white, white, black, white, white];
  }
  if (locX > 0.5 && locY > 0.75) {
    cellInnerCoords = [white, white, white, white, white, white, white, black];
  }
...
document.addEventListener("mousemove", function (e) {
  if (e.ctrlKey) {
    ...
    // Draw a solid block instead of using PixelBrailleCharacter
    if (edge && char !== "undefined") {
      writeCharTo("█", YourWorld.Color, x, y, z, w);
    }
  }
});
